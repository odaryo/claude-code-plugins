...
**入力:**
```
/consulting-codex discuss: ユーザー認証のセッション管理方式。要件: SPA + API 構成で、モバイルアプリからも利用する
```

**期待される動作:**
- モード: `discuss`
- Claude が初期案を作成してユーザーに表示してから議論開始
- 初回: 新規 `codex exec`（要件＋案＋共通レビュー観点＋【懸念なし/あり】出力制約）
- 2回目以降: `codex exec resume <session-id>`（指摘への対応＋改訂版）
- 各往復後に進捗1行サマリが表示される
- 終了条件: 【懸念なし】宣言 / 指定回数 / ハード上限10往復

**検証ポイント:**
- [ ] 「議論して」「懸念がなくなるまで」キーワードでも discuss が選択される
- [ ] `discuss(N):` で往復上限 N が適用される
- [ ] 3往復超過後は Critical/High のみ対応継続（妥協フェーズ）になる
- [ ] 最終出力に「最終成果物」「合意状態」「残懸念」「議論サマリ」が含まれる
- [ ] Codex の指摘に対し、Claude が反映だけでなく根拠付き反論もできている
...
| arch/opinion通常 | 1 | 0 |
| resume（全モード） | 1 | 0-1 |
| discuss（N往復） | N（初回exec + resume×N-1） | 0-1 |

※ 期待Bash は成功時の回数。codex 呼び出しが失敗した場合のみ再試行1回（+1）が許容される。
